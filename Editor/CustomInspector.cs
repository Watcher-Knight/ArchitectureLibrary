...
namespace ArchitectureLibrary.Editor
{
    public class CustomInspector : UnityEditor.Editor
    {
        public T GetField<T>(params string[] path)
        {
            ...
            {
                FieldInfo field = targetObject.GetType().GetField(f, SerializedProperties.BindingFlags);
                targetObject = field.GetValue(targetObject);
            }
            return SerializedProperties.Convert<T>(targetObject);
        }

        public void SetField(object value, params string[] path)
        {
            object targetObject = target;
            FieldInfo field = targetObject.GetType().GetField(path[0], SerializedProperties.BindingFlags);
            for (int i = 1; i > path.Length - 1; i++)
            {
                targetObject = field.GetValue(targetObject);
                field = targetObject.GetType().GetField(path[i], SerializedProperties.BindingFlags);
            }
        ...
        {
            IEnumerable<PropertyInfo> properties = target.GetType().GetProperties(SerializedProperties.BindingFlags);
        ...
        {
            IEnumerable<MethodInfo> methods = target.GetType().GetMethods(SerializedProperties.BindingFlags);
